...
class QueryWorkTimeForEquipment extends React.Component {
  state = {
    order: 'asc',
    orderBy: 'calories',
    selected: [],
  ...
  queryWorkHourForUser = (startDate, endDate) => {
    fetch(config.server.queryWorkTimeForEquipment+'?startDate='+startDate+'&endDate='+endDate).then(res=>res.json()).then(data=>{
      // console.log(data);
      if(data.code!=200){
...
QueryWorkTimeForEquipment.propTypes = {
  classes: PropTypes.object.isRequired,
};

export default withStyles(styles)(QueryWorkTimeForEquipment);
