...
class EnhancedTableToolbar extends React.Component{
  ...
  state = {
    ...
    keyword: sessionStorage.indentStatusKeywordSure?(sessionStorage.indentStatusKeyword || ''):''
  }
  ...
  queryKeyword = (e) =>{
    ...
    this.props.queryByKeyword(e.target.value);
  }
...
const styles = theme => ({
  ...
  tableWrapper: {
    height:"60vh",overflow:"auto",width: "fit-contentoo"
  },
  markTd: {
    overflow: "hidden",
...
class QueryIndentStatus extends React.Component {
  ...
  infoTarget = (n)=>{
    sessionStorage.indentStatusKeywordSure = 1;
    this.props.history.push('/produceMSF/indent/info'+n.id);
  }

  setProcedureRestart = (item, restart) =>{
    var nexFun = ()=>{
      // 设置流程为开始/重新开始
      fetch(config.server.updateDutyProcedureStatus,{method:"POST",
        ...
        restart?this.tips('该流程已重新开始'):this.tips('该流程已开始');
        // this.state.data[index].status = 1;
        item.status = 1;
      }).catch(e=>{console.log(e);this.tips('网络出错了，请稍候再试')});

    };
    var innerMsg = restart?('订单<span class="text-blue">'+item.erp+'</span>的<span class="text-blue">'+item.name+'</span>流程已完成，确定重新开始吗？'):('确定开始订单<span class="text-blue">'+item.erp+'</span>的<span class="text-blue">'+item.name+'</span>流程吗？');
    this.setState({confirmOpen: true,title: "确认",content: innerMsg,sureFun: nexFun, ifInfo: true});
  }
