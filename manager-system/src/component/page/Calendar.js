...
class WorkCalendar extends React.Component {
  ...
  initLabelClick = () =>{
    ...
    this.clickEvent = document.addEventListener('click', (e)=>{
      var { workDate, workArr, changeDate } = this.state;
      e.stopPropagation();
      e.preventDefault();
      // console.log(workDate);
  ...
  uploadWorkCalendar = () => {
    ...
    for(var i in workDate){
      if(new Date(i).getTime()>=cur_time){
        upload_date[i] = workDate[i]
      }
    }
    /*for(var i of changeDate){
        upload_date[i] = workDate[i];
    }*/
    ...
    fetch(config.server.updateWorkCalendar,{method:"POST",
      ...
      body:JSON.stringify({workDate: upload_date})
