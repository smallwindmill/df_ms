...
class QueryUser extends React.Component {
  ...
  queryByKeyword = (e) =>{
    ...
    if(!this.state.dataBak){
      this.state.dataBak = this.state.userd;
    }

    this.state.data = this.state.dataBak.filter((item)=>{
      return (item.userID.toUpperCase().indexOf(e.target.value.toUpperCase())!=-1 || item.userName.toUpperCase().indexOf(e.target.value.toUpperCase())!=-1);
    });
    this.setState({data: this.state.data});
  }
  ...
  render() {
    ...
    return (
      <Paper className={classes.root} style={{padding:"0 2rem",width:"auto"}}>
        <EnhancedTableToolbar changeUserData = {this.changeUserData} queryUserByType = {this.queryUserByType} ref="modalMethod" tips = {this.tips} queryByKeyword = {this.queryByKeyword} />
