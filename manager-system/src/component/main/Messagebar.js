...
class MessageBar extends React.Component{
    ...
    componentDidMount() {
      // 更新消息数据
      this.queryMessage();
      // this.state.messageTimer = setInterval(this.queryMessage, 20000);//首次加载判断时触发，不放在此处
    ...
    markMessage = (data, index) =>{
      ...
      fetch(config.server.markMessageRead,{method:"POST",
        ...
        body:JSON.stringify({id: data=="all"?data:data.id})
      }).then(res=>res.json()).then(data_res=>{
        if(data_res.code!=200){
          this.props.tips(data_res.msg);return;
        }
        if(data=="all"){
          this.queryMessage();
          return;
